...
export async function sendSMS(phone: string, message: string) {
    try {
        ...
        const brandName = process.env.ESMS_BRAND_NAME ? process.env.ESMS_BRAND_NAME.trim() : ""
        ...
        let url = `https://rest.esms.vn/MainService.svc/json/SendMultipleMessage_V4_get?Phone=${encodeURIComponent(formattedPhone)}&Content=${encodeURIComponent(message)}&ApiKey=${apiKey}&SecretKey=${secretKey}&IsUnicode=0&SmsType=${smsType}`
